...
llvm::Type *ABITypeMapper::convertType(const abi::Type *ABIType) {
  ...
  case abi::TypeKind::Pointer:
    Result = llvm::PointerType::get(Context,
...
llvm::Type *ABITypeMapper::convertArrayType(const abi::ArrayType *AT) {
  ...
  if (AT->isMatrixType())
    return llvm::VectorType::get(ElementType, ElementCount::getFixed(NumElements));
...
llvm::Type *ABITypeMapper::convertRecordType(const abi::RecordType *RT) {
  ...
  return createStructFromFields(FieldsArray,
                                RT->getSizeInBits(), RT->getAlignment(),
                                RT->isUnion(), RT->isCoercedRecord());
}
...
llvm::StructType *ABITypeMapper::createStructFromFields(
   ArrayRef<abi::FieldInfo> Fields, TypeSize Size,
 ...
 if (IsUnion) {
   ...
   if (LargestFieldType) {
     ...
     if (LargestFieldSize < UnionSizeBits) {
       llvm::Type *PaddingType = createPaddingType(UnionSizeBits - LargestFieldSize);
       if (PaddingType)
   ...
   for (const auto &Field : Fields) {
     if (!IsCoercedStr && Field.OffsetInBits > CurrentOffset) {
       llvm::Type *PaddingType = createPaddingType(Field.OffsetInBits - CurrentOffset);
       if (PaddingType)
         FieldTypes.push_back(PaddingType);
   ...
   if (!IsCoercedStr) {
     ...
     if (CurrentOffset < TotalSizeBits) {
       llvm::Type *PaddingType = createPaddingType(TotalSizeBits - CurrentOffset);
       if (PaddingType)
         FieldTypes.push_back(PaddingType);
     }
   }
